...
# Draw a box with dimensions 120x160px
# with vertical divisions every 40px
def box(size):
    # Setup
    t.pd()
    x = 90
...
def next(size):
    # move turtle to next letter
    t.pu()
    t.fd(140 * size)


def nextB(size):
    # move turtle to next letter for b and c
    t.pu()
    t.fd(120 * size)
...
def nextP(size):
    # move turtle to nt.fd(size * 30)ext letter for p
    t.pu()
    t.fd(100 * size)
